...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls,
  Buttons, ExtCtrls, ComCtrls, LdapClasses, ImgList, Vcl.Grids, System.UITypes;
...
begin
  ...
  { Setup grid }
  with TimeGrid do begin
    ColWidths[0] := Width - 2 * BevelWidth - 24 * (DefaultColWidth + GridLineWidth) - 2 * GridLineWidth;
    for i := 1 to 6 do
      Cells[0, i] := FormatSettings.LongDayNames[i + 1];
    Cells[0, 7] := FormatSettings.LongDayNames[1];
    { Print hours }
    tp := ColWidths[0] + GridLineWidth + BevelWidth;
    i := 0;
    while i < 25 do
    begin
      s := IntToStr(i);
...
begin
  ...
  begin
    Rect.Left := Rect.Left - (DefaultColWidth - Rect.Width);
