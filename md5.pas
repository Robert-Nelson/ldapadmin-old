...
interface
  ...
  procedure md5digest(pMessage: Pointer; len: word; var d: digest);
    {- digest a message with length len bytes, put the result in d }

implementation

  type context = array[0..3] of Cardinal;

  var x: array[0..15] of Cardinal;
      ctxt: context;

  function f(x,y,z: Cardinal): Cardinal; far;
  begin
    f := (x and y) or ((not x) and z)
  end;

  function g(x,y,z: Cardinal): Cardinal; far;
  begin
    g := (x and z) or (y and (not z))
  end;

  function h(x,y,z: Cardinal): Cardinal; far;
  begin
    h := x xor y xor z
  end;

  function i(x,y,z: Cardinal): Cardinal; far;
  begin
    i := y xor (x or (not z))
  end;

  function rol(x: Cardinal; s: byte): Cardinal;
  begin
    {$ifdef bp7bug}
    for s := 1 to s do
      if x >= 0
  ...
  procedure transform;
    type fn = function(x,y,z: Cardinal): Cardinal;
