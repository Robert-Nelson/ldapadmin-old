...
uses Classes, Windows, LDAPClasses, WinLDAP, Constant, Connection, ComCtrls, UITypes, jpeg;
...
uses
  ...
  Pickup, Dialogs, Forms, ActiveX, ComObj, ActiveDs_TLB, adsie, AclApi, AccCtrl;
...
begin
  ...
  begin
    { According to MSDN documentation of FILETIME struct, we may not simply
      cast dt as UINT64 as it may cause alignment faults on 64-bit Windows }
    uli.QuadPart := StrToInt64(s);
    dt.dwLowDateTime := uli.LowPart;
    dt.dwHighDateTime := uli.HighPart;
...
begin
  with AttributesByName['accountExpires'] do
    if Value = 0 then
      ...
      { See above comment on casting of TFileTime }
      dt := DateTimeToWinApiTime(LocalDateTimeToUTC(Value));
      uli.LowPart := dt.dwLowDateTime;
      uli.HighPart := dt.dwHighDateTime;
      AsString := UIntToStr(uli.QuadPart);
    end;
end;
